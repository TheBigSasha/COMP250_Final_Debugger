...
public class HashTableBenchmark {
    MyHashTable<String, Tweet> table;
    RandomTweets rand;

    public HashTableBenchmark() {
        table = new MyHashTable<String, Tweet>(10);
    ...
    public long timedPut(Tweet input) {
        ...
        table.put(input.getDateAndTime(), input);
    ...
    public long timedPut(int howMany) {
        ...
        for (int i = 0; i < tweets.length - 1; i++) {
            table.put(tweets[i].getDateAndTime(), tweets[i]);
        }
        long startTime = System.nanoTime();
        table.put(tweets[tweets.length - 1].getDateAndTime(), tweets[tweets.length - 1]);
        long endTime = System.nanoTime();
        return endTime - startTime;
    }
